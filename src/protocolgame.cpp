...
void ProtocolGame::login(const std::string& name, uint32_t accountId, OperatingSystem_t operatingSystem)
{
	...
	if (!foundPlayer || g_config.getBoolean(ConfigManager::ALLOW_CLONES)) {
		...
		{
			if (player->getIP() == it.second->getIP() && ++ipsCounter >= 3)
			{
				disconnectClient("Voce so pode fazer login com 3 personagens por IP, e voce atingiu o limite..");
				return;
			}
		}
...
void ProtocolGame::GetTileDescription(const Tile* tile, NetworkMessage& msg)
{
	...
	if (ground) {
		...
		if (it.clientId < MAX_CLIENT_ID_860) {
			msg.addItem(ground);
			count = 1;
		} else {
			std::cout << "[Warning - GetTileDescription] Skipping ground with clientId " << it.clientId 
			          << " (exceeds limit " << MAX_CLIENT_ID_860 << ") at position " << tile->getPosition() << std::endl;
			count = 0;
		}
	} else {
		count = 0;
	...
	if (items) {
		for (auto it = items->getBeginTopItem(), end = items->getEndTopItem(); it != end; ++it) {
			...
			if (itemType.clientId < MAX_CLIENT_ID_860) {
				msg.addItem(*it);
				...
				std::cout << "[Warning - GetTileDescription] Skipping top item with clientId " << itemType.clientId 
				          << " (exceeds limit " << MAX_CLIENT_ID_860 << ") at position " << tile->getPosition() << std::endl;
			}
		}
	}
	...
	if (items && count < 10) {
		for (auto it = items->getBeginDownItem(), end = items->getEndDownItem(); it != end; ++it) {
			...
			if (itemType.clientId < MAX_CLIENT_ID_860) {
				...
				std::cout << "[Warning - GetTileDescription] Skipping down item with clientId " << itemType.clientId 
				          << " (exceeds limit " << MAX_CLIENT_ID_860 << ") at position " << tile->getPosition() << std::endl;
			}
		}
	}
...
void ProtocolGame::sendContainer(uint8_t cid, const Container* container, bool hasParent, uint16_t firstIndex)
{
	...
	const ItemType& containerType = Item::items[container->getID()];
	if (containerType.clientId >= MAX_CLIENT_ID_860) {
		std::cout << "[Warning - sendContainer] Skipping container with clientId " << containerType.clientId 
		          << " (exceeds limit " << MAX_CLIENT_ID_860 << ")" << std::endl;
		return;
	}
	...
	for (ItemDeque::const_iterator cit = itemList.begin() + firstIndex, end = itemList.end(); i < 0xFF && cit != end; ++cit, ++i) {
		const ItemType& itemType = Item::items[(*cit)->getID()];
		if (itemType.clientId < MAX_CLIENT_ID_860) {
			msg.addItem(*cit);
		} else {
			std::cout << "[Warning - sendContainer] Skipping item with clientId " << itemType.clientId 
			          << " (exceeds limit " << MAX_CLIENT_ID_860 << ") in container " << (int)cid << std::endl;
		}
	}
	writeToOutputBuffer(msg);
}
...
void ProtocolGame::sendTradeItemRequest(const std::string& traderName, const Item* item, bool ack)
{
	...
	if (itemType.clientId >= MAX_CLIENT_ID_860) {
		std::cout << "[Warning - sendTradeItemRequest] Skipping item with clientId " << itemType.clientId 
		          << " (exceeds limit " << MAX_CLIENT_ID_860 << ")" << std::endl;
		return;
	}
	...
	if (const Container* tradeContainer = item->getContainer()) {
		...
		for (const Item* listItem : itemList) {
			const ItemType& listItemType = Item::items[listItem->getID()];
			if (listItemType.clientId < MAX_CLIENT_ID_860) {
				msg.addItem(listItem);
			} else {
				std::cout << "[Warning - sendTradeItemRequest] Skipping item with clientId " << listItemType.clientId 
				          << " (exceeds limit " << MAX_CLIENT_ID_860 << ") in trade" << std::endl;
			}
		}
	} else {
		msg.addByte(0x01);
...
void ProtocolGame::sendAddTileItem(const Position& pos, uint32_t stackpos, const Item* item)
{
	...
	if (it.clientId >= MAX_CLIENT_ID_860) {
		std::cout << "[Warning - sendAddTileItem] Skipping item with clientId " << it.clientId 
		          << " (exceeds limit " << MAX_CLIENT_ID_860 << ") at position " << pos << std::endl;
		return;
	}
...
void ProtocolGame::sendUpdateTileItem(const Position& pos, uint32_t stackpos, const Item* item)
{
	...
	const ItemType& it = Item::items[item->getID()];
	if (it.clientId >= MAX_CLIENT_ID_860) {
		std::cout << "[Warning - sendUpdateTileItem] Skipping item with clientId " << it.clientId 
		          << " (exceeds limit " << MAX_CLIENT_ID_860 << ") at position " << pos << std::endl;
		return;
	}
...
void ProtocolGame::sendInventoryItem(slots_t slot, const Item* item)
{
	...
	if (item) {
		const ItemType& it = Item::items[item->getID()];
		if (it.clientId >= MAX_CLIENT_ID_860) {
			std::cout << "[Warning - sendInventoryItem] Skipping item with clientId " << it.clientId 
			          << " (exceeds limit " << MAX_CLIENT_ID_860 << ") in slot " << slot << std::endl;
			msg.addByte(0x79);
			msg.addByte(slot);
		} else {
			msg.addByte(0x78);
...
void ProtocolGame::sendAddContainerItem(uint8_t cid, const Item* item)
{
	const ItemType& it = Item::items[item->getID()];
	if (it.clientId >= MAX_CLIENT_ID_860) {
		std::cout << "[Warning - sendAddContainerItem] Skipping item with clientId " << it.clientId 
		          << " (exceeds limit " << MAX_CLIENT_ID_860 << ") in container " << (int)cid << std::endl;
		return;
	}
...
void ProtocolGame::sendUpdateContainerItem(uint8_t cid, uint16_t slot, const Item* item)
{
	const ItemType& it = Item::items[item->getID()];
	if (it.clientId >= MAX_CLIENT_ID_860) {
		std::cout << "[Warning - sendUpdateContainerItem] Skipping item with clientId " << it.clientId 
		          << " (exceeds limit " << MAX_CLIENT_ID_860 << ") in container " << (int)cid << " slot " << slot << std::endl;
		return;
	}
...
void ProtocolGame::sendTextWindow(uint32_t windowTextId, Item* item, uint16_t maxlen, bool canWrite)
{
	const ItemType& it = Item::items[item->getID()];
	if (it.clientId >= MAX_CLIENT_ID_860) {
		std::cout << "[Warning - sendTextWindow] Skipping item with clientId " << it.clientId 
		          << " (exceeds limit " << MAX_CLIENT_ID_860 << ")" << std::endl;
		return;
	}
